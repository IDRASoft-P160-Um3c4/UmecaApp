app.controller('personalDataController', function ($scope, $timeout, $q, $http, $rootScope, $sce) {
    ...
    $scope.initSelBirthInfo = function () {
        if ($scope.lstInfoAvail === undefined || $scope.lstInfoAvail.length <= 0)
            return;

        if ($scope.m.birthInfoId > 0) {
            for (var i = 0; i < $scope.lstInfoAvail.length; i++) {
                if ($scope.lstInfoAvail[i].id == $scope.m.birthInfoId) {//para seleccionar a mexico por defecto
                    $scope.m.birthInfo = $scope.lstInfoAvail[i];
                    break;
                }
            }
        } else {
