...
public class MeetingServiceImpl implements MeetingService {
    ...
    public Long createMeeting(Imputed imputed) {
        ...
        try {
            ...
            if(imputed.getIsFromFormulation() == true){
                ...
                eventService.addEvent(Constants.EVENT_FROM_FORMULATION,meeting.getCaseDetention().getId(),null);
            }
    ...
    public ModelAndView showMeeting(Long id) {
        ...
        String[] arrProps = new String[]{"imputed.name", "imputed.lastNameP", "imputed.lastNameM"};
        m = (Meeting) StringEscape.escapeAttrs(m, arrProps);
    ...
    public ModelAndView showLegalProcess(Long id, Integer showCase) {
        ...
        String[] arrProps = new String[]{"idFolder", "meeting.imputed.name", "meeting.imputed.lastNameP", "meeting.imputed.lastNameM"};
        c = (Case) StringEscape.escapeAttrs(c, arrProps);
    ...
    @Transactional
    @Override
    public ResponseMessage doTerminateMeeting(Meeting meeting, String sch, String activities, Boolean cancelMeeting) {
        ResponseMessage result = new ResponseMessage();
        try {
            ...
            if (c != null && c.size() > 0) {
                for (Case cAux : c) {
                    ...
                    if (iCase.getFoneticString().equals(imputed.getFoneticString()) && iCaseBD.equals(imputed.getBirthDate().getTime())) {
                        return new ResponseMessage(true, "El n&uacute;mero de carpeta de investigaci&oacute;n y el imputado ya se encuentran registrados.");
                    }
                }
            }
        } else {
    ...
    public void refreshCurrentProceeding(CriminalProceedingView cpv, Case c) {
        ...
        if (listOldCrime != null) {
            ccpc.setCrimeList(null);
            for (Crime curr : listOldCrime) {
                curr.setCriminalProceeding(null);
            }
        ...
        if (listOldCoDefendant != null) {
            ccpc.setCoDefendantList(null);
            for (CoDefendant curr : listOldCoDefendant) {
                curr.setCriminalProceeding(null);
            }
        ...
        if (cpv.getIsFolderAccess() == false) {
            ...
            ccpc.setLocationDetention(locationRepository.findByLocName(Constants.COUNTRY_STATE_MUNICIPALITY_LOCATION_NOT_KNWOW));
            ccpc.setIsFolderAccess(cpv.getIsFolderAccess());
        } else {
            ccpc.setIsFolderAccess(cpv.getIsFolderAccess());
