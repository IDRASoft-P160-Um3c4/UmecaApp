...
public class FramingMeetingServiceImpl implements FramingMeetingService {
    ...
    public ResponseMessage deleteFramingAddress(Long id) {
        ...
        try {
            ...
            return new ResponseMessage(false, "Se ha eliminado el registro con �xito.");
        } catch (Exception e) {
    ...
    public ResponseMessage deleteReference(Long id) {
        ...
        try {
            ...
            if (framingSelectedSourceRelRepository.findSourceRelByIdSource(id) != null)
                return new ResponseMessage(true, "No se puede eliminar la informaci�n. El registro ha sido seleccionado en la secci�n An�lisis del entorno.");
    ...
    public ResponseMessage doUpsertDrug(Drug drug, Long idCase) {
        ...
        try {
            ...
            result.setMessage("Se ha guardado la informaci�n con �xito");
        } catch (Exception e) {
            logException.Write(e, this.getClass(), "doUpsertDrug", sharedUserService);
            result.setHasError(true);
            result.setMessage("Ocurrio un error al guardar la informaci�n. Int�nte m�s tarde.");
        }
        return result;
    }
    ...
    public ResponseMessage deleteDrug(Long id) {
        ...
        try {
            ...
            result.setHasError(false);
            result.setMessage("Se elimino la sustancia con �xito");
        } catch (Exception e) {
            logException.Write(e, this.getClass(), "deleteDrug", sharedUserService);
            result.setHasError(true);
            result.setMessage("Ocurrio un error al eliminar la sustancia. Int�nte m�s tarde");
        }
        return result;
    }
        ...
        try {
            ...
            if (existFraming.getPersonalData() == null)
                if (sb.toString().equals(""))
                    sb.append("Debe proporcionar la informaci�n faltante para la secci�n \"Datos personales y entorno social\".");
                else
                    sb.append("|Debe proporcionar la informaci�n faltante para la secci�n \"Datos personales y entorno social\".");

            if (existFraming.getFramingAddresses() == null || !(existFraming.getFramingAddresses().size() > 0))
                if (sb.toString().equals(""))
                    sb.append("Debe registrar al menos un registro en la secci�n \"Domicilios\".");
                else
                    sb.append("|Debe registrar al menos un registro en la secci�n \"Domicilios\".");

            if (existFraming.getProcessAccompaniment() == null)
                if (sb.toString().equals(""))
                    sb.append("Debe proporcionar la informaci�n faltante para la secci�n \"Persona que acompa�a en el proceso\".");
                else
                    sb.append("|Debe proporcionar la informaci�n faltante para la secci�n \"Persona que acompa�a en el proceso\".");
            ...
            if (existFraming.getReferences() != null && existFraming.getReferences().size() > 0) {
                ...
                if (noHousemate == 0)
                    if (sb.toString().equals(""))
                        sb.append("Debe registrar al menos una registro en en la secci�n \"Personas que viven con el imputado\".");
                    else
                        sb.append("|Debe registrar al menos una registro en en la secci�n \"Personas que viven con el imputado\".");
                if (noReferences == 0)
                    if (sb.toString().equals(""))
                        sb.append("Debe registrar al menos una registro en en la secci�n \"Referencias personales\".");
                    else
                        sb.append("|Debe registrar al menos una registro en en la secci�n \"Referencias personales\".");
            ...
            if (existFraming.getOccupation() == null && existFraming.getActivities() == null)
                if (sb.toString().equals(""))
                    sb.append("Debe proporcionar la informaci�n faltante para la secci�n \"Actividades que realiza el imputado\".");
                else
                    sb.append("|Debe proporcionar la informaci�n faltante para la secci�n \"Actividades que realiza el imputado\".");
            if (existFraming.getDrugs() == null || !(existFraming.getDrugs().size() > 0))
                if (sb.toString().equals(""))
                    sb.append("Debe registrar al menos una registro en en la secci�n \"Consumo de substancias\".");
                else
                    sb.append("|Debe registrar al menos una registro en en la secci�n \"Consumo de substancias\".");
            if (existFraming.getSelectedSourcesRel() == null || !(existFraming.getSelectedSourcesRel().size() > 0) ||
                ...
                if (sb.toString().equals(""))
                    sb.append("Debe proporcionar la informaci�n faltante para la secci�n \"An�isis del entorno\".");
                else
                    sb.append("|Debe proporcionar la informaci�n faltante para la secci�n \"An�isis del entorno\".");
            if (existFraming.getAdditionalFramingQuestions() == null)
                if (sb.toString().equals(""))
                    sb.append("Debe proporcionar la informaci�n faltante para la secci�n \"Formulario de preguntas al supervisado\".");
                else
                    sb.append("|Debe proporcionar la informaci�n faltante para la secci�n \"Formulario de preguntas al supervisado\".");
            if (!sb.toString().equals("")) {
            ...
            return new ResponseMessage(false, "Ha ocurrido un error al guardar la informaci�n. Intente m�s tarde.");
        }
    }
