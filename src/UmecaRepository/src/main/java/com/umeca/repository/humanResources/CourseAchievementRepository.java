...
public interface CourseAchievementRepository extends JpaRepository<CourseAchievement, Long> {
    ...
    @Query("select new com.umeca.model.dto.humanResources.CourseAchievementDto(CA.id, E.id, CA.name, CA.place,CA.duration,CA.start,CA.end,GF.id) from CourseAchievement CA " +
            "inner join CA.employee E " +
            "inner join CA.file GF "+
            "where CA.id=:idCourse and E.id = :idEmployee")
