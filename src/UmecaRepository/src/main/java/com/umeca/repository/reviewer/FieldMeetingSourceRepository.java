...
public interface FieldMeetingSourceRepository extends JpaRepository<FieldMeetingSource, Long> {
    ...
    @Query("select new com.umeca.model.dto.FieldValueBySource(fms.id, sv.id, fv.code, fv.section, fv.sectionCode, fv.fieldName, fv.idSubsection, fms.value, fms.idFieldList) " +
            "from Case as c " +
            "INNER JOIN c.verification.sourceVerifications sv " +
            "INNER JOIN sv.fieldMeetingSourceList as fms " +
            "INNER JOIN fms.fieldVerification as fv " +
            "where c.id = :idCase and sv.id=:sourceId")
    List<FieldValueBySource> getInfoBySource(@Param("idCase") Long idCase, @Param("sourceId") Long sourceId);
    ...
    @Query("select new com.umeca.model.dto.FieldValueBySource(fms.id, sv.id, fv.code, fv.section, fv.sectionCode, fv.fieldName, fv.idSubsection, fms.value, fms.idFieldList) " +
            ...
            "INNER JOIN sv.fieldMeetingSourceList as fms " +
            "INNER JOIN fms.fieldVerification as fv " +
            "where c.id = :idCase and fms.isFinal=true and fms.reason is not null")
    List<FieldValueBySource> getFinalInfoByCase(@Param("idCase") Long idCase);
