...
public interface StatisticSupervisorManagerReportRepository extends JpaRepository<StatisticSupervisorManagerReportType, Long> {
    ...
    @Query("select new com.umeca.model.shared.SelectList(count(ev.id)) " +
            ...
            "where evT.name = com.umeca.model.shared.Constants.EVENT_PROSECUTE and (ev.dateId between :initDate and :endDate) and c.district.id = :idDistrict")
    List<SelectList> countCasesProsecutedByDistrict(@Param("initDate") Integer initDate, @Param("endDate") Integer endDate, @Param("idDistrict") Long idDistrict);
    ...
    @Query(value = "SELECT cat_event.event, count(event.id_event) FROM cat_event " +
            ...
            "                on event.id_case = case_detention.id_case " +
            "                WHERE (cat_event.event = 'OPINION' or " +
            "                cat_event.event = 'DECLINED' or " +
            "                cat_event.event = 'REPORT' ) " +
            "                and event.id_case = :idCase " +
            ...
            "                group by cat_event.id_event", nativeQuery = true)
    List<Object> countEventsByCaseByDistrict(@Param("idCase") Long idCase, @Param("initDate") Integer initDate, @Param("endDate") Integer endDate, @Param("idDistrict") Long idDistrict);
...
//            "and cahear.isFinished = true " +
//            "and cahear.hearingFormatSpecs.arrangementType = 2 " +
//            "and ca.status.name in ('ST_CASE_HEARING_FORMAT_EqND' , 'ST_CASE_FRAMING_MEETING_INCOMPLETE', 'ST_CASE_FRAMING_MEETING_COMPLETE', 'ST_CASE_REQUEST', 'ST_CASE_REQUEST_SUPERVISION','ST_CASE_CLOSE_REQUEST') " +
//            "and ca.district.id = :districtId " +
//            "and ca.umecaSupervisor.id = :supervisorId " +
//            "order by cahear.id desc")
//    List<SelectList> countSCPPByDistrictAndSupervisor(@Param("initDate") Date initDate, @Param("endDate") Date endDate, @Param("districtId") Long districtId, @Param("supervisorId") Long supervisorId);
        ...
        @Query("select new com.umeca.model.shared.SelectList('SCPP', count(distinct ca.id)) " +
                "from Case ca " +
                "inner join ca.status stc " +
                "inner join ca.district ds " +
                "inner join ca.umecaSupervisor sup " +
                "inner join ca.hearingFormats hf " +
                "inner join hf.hearingFormatSpecs hfs " +
                "where ca.dateCreate between :initDate and :endDate " +
                "and hf.isFinished = true " +
                "and hfs.arrangementType = 2 " +
                "and stc.name in ('ST_CASE_HEARING_FORMAT_END' , 'ST_CASE_FRAMING_MEETING_INCOMPLETE', 'ST_CASE_FRAMING_MEETING_COMPLETE', 'ST_CASE_REQUEST', 'ST_CASE_REQUEST_SUPERVISION','ST_CASE_CLOSE_REQUEST') " +
                "and ds.id = :districtId " +
                "and sup.id = :supervisorId " +
                "order by ca.id desc")
        List<SelectList> countSCPPByDistrictAndSupervisor(@Param("initDate") Date initDate, @Param("endDate") Date endDate, @Param("districtId") Long districtId, @Param("supervisorId") Long supervisorId);
    ...
    List<SelectList> countSubstractedByDistrictAndSupervisorNull(@Param("initDate") Date initDate, @Param("endDate") Date endDate, @Param("districtId") Long districtId);
        ...
        @Query(value = "select supervision_activity.description, count(ResA.sub_id_case) from supervision_activity " +
                "left join activity_monitoring_plan " +
                "on activity_monitoring_plan.id_supervision_activity = supervision_activity.id_supervision_activity " +
                "and supervision_activity.code = 'VIDO' " +
                ...
                ") ResA " +
                "on activity_monitoring_plan.id_case = ResA.sub_id_case" ,nativeQuery = true)
        List<Object> countHomeVisitsGeneral(@Param("initDate") String initDate, @Param("endDate") String endDate);
        ...
        @Query(value = "select supervision_activity.description, count(ResA.sub_id_case) from supervision_activity " +
                ...
                "inner join framing_meeting on framing_meeting.id_case = case_detention.id_case and framing_meeting.is_terminated = true " +
                ") ResA " +
                "on activity_monitoring_plan.id_case = ResA.sub_id_case and ResA.sub_id_district = :idDistrict ", nativeQuery = true)
        List<Object> countHomeVisitsByDistrict(@Param("initDate") String initDate, @Param("endDate") String endDate, @Param("idDistrict") Long idDistrict);
        ...
        @Query(value = "select supervision_activity.description, count(ResA.sub_id_case) from supervision_activity " +
                ...
                "inner join framing_meeting on framing_meeting.id_case = case_detention.id_case and framing_meeting.is_terminated = true " +
                ") ResA " +
                "on activity_monitoring_plan.id_case = ResA.sub_id_case and ResA.sub_id_district = :idDistrict and ResA.sub_id_umeca_supervisor = :idSupervisor ", nativeQuery = true)
        List<Object> countHomeVisitsBySupervisor(@Param("initDate") String initDate, @Param("endDate") String endDate, @Param("idDistrict") Long idDistrict, @Param("idSupervisor") Long idSupervisor);
