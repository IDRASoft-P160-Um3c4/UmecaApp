...
public interface CaseRepository extends JpaRepository<Case, Long> {
    ...
    @Query("select  new com.umeca.model.entities.reviewer.FindLegalBefore(c.id,c.idMP,c.idFolder,s.description,c.dateCreate,concat(i.name,' ',i.lastNameP,' ',i.lastNameM),i.birthDate) from Case as c " +
            ...
            "INNER JOIN c.meeting.status as mes " +
            "where i.foneticString=:foneticString and c.id <> :idCase " +
            " and mes.name <> 'OBSOLETE' ")
...
//            "left join CCP.domicileVictim AV " +
    ...
    @Query("SELECT COUNT(C) FROM Case C INNER JOIN C.status ST INNER JOIN C.framingMeeting FM " +
            "WHERE C.id =:caseId AND FM.isTerminated = 1 AND ST.name <> '" + Constants.CASE_STATUS_CLOSED + "'")
    Long isReadyForChanneling(@Param("caseId") Long caseId);
...
//            "and sm.name= com.umeca.model.shared.Constants.S_MEETING_INCOMPLETE")
    ...
    @Query("select new com.umeca.model.entities.supervisorManager.ExcelCaseInfoSupDto( " +
            ...
            "left join FMPERDATA.birthCountry BIRTHCOUNTRY " +
            "left join FMPERDATA.birthStateCmb BIRTHSTATE " +
            "left join FMPERDATA.maritalStatus MARSTATUS " +
            "left join FM.school FMSCHOOL " +
    ...
    @Query("select new com.umeca.model.entities.supervisor.ExcelImputedHomeDto(CDET.id,FMADD.isHomeless, LOC.zipCode,ST.name,MUN.name,HT.name,RT.name) " +
            "from Case CDET " +
            "inner join CDET.framingMeeting FM " +
            "left join FM.framingAddresses FMADD " +
            "left join FMADD.address ADDR " +
            "left join ADDR.location LOC " +
            "left join LOC.municipality MUN " +
            "left join MUN.state ST " +
            "left join FMADD.homeType HT " +
    ...
    @Query("select new com.umeca.model.entities.supervisor.ExcelReferenceDto(CDET.id,FMREF.age, FMREF.isAccompaniment, FMREFRELS.name ) " +
            "from Case CDET " +
            "inner join CDET.framingMeeting FM " +
            "left join FM.references FMREF " +
            "left join FMREF.relationship FMREFRELS " +
            "where CDET.id in (:casesIds) and FMREF.personType = com.umeca.model.entities.supervisor.FramingMeetingConstants.PERSON_TYPE_HOUSEMATE  ")
    List<ExcelReferenceDto> getHousematesSup(@Param("casesIds") List<Long> lstCasesIds);
    ...
    @Query("select new com.umeca.model.entities.supervisor.ExcelJobDto(CDET.id, JOB.block, JRTY.name) " +
            "from Case CDET " +
            "inner join CDET.framingMeeting FM " +
            "left join FM.jobs JOB " +
            "left join JOB.registerType JRTY " +
            "where CDET.id in (:casesIds) ")
    List<ExcelJobDto> getJobSup(@Param("casesIds") List<Long> lstCasesIds);


    @Query("select new com.umeca.model.entities.supervisor.ExcelActivitiesDto(CDET.id, ACT.name, FMACT.description) " +
            "from Case CDET " +
            "inner join CDET.framingMeeting FM " +
            "left join FM.activities FMACT " +
            "left join FMACT.activity ACT " +
            "where CDET.id in (:casesIds) ")
    List<ExcelActivitiesDto> getActivitiesSup(@Param("casesIds") List<Long> lstCasesIds);




    @Query("select new com.umeca.model.entities.supervisor.ExcelDrugDto(CDET.id, DRUGTYPE.name,PERIOD.name, DRUG.onsetAge) " +
            "from Case CDET " +
            "inner  join CDET.framingMeeting FM " +
            "left join FM.drugs DRUG " +
            "left join DRUG.drugType DRUGTYPE " +
            "left join DRUG.periodicity PERIOD " +
            "where CDET.id in (:casesIds) ")
    List<ExcelDrugDto> getDrugsSup(@Param("casesIds") List<Long> lstCasesIds);


    @Query("select new com.umeca.model.entities.supervisor.ExcelRiskDto(CDET.id, RISK.description) " +
            "from Case CDET " +
            "inner join CDET.framingMeeting FM " +
            "left join FM.selectedRisksRel SELRISK " +
            "left join SELRISK.framingRisk RISK " +
            "where CDET.id in (:casesIds) ")
    List<ExcelRiskDto> getRiskSup(@Param("casesIds") List<Long> lstCasesIds);




    @Query("select new com.umeca.model.entities.supervisor.ExcelThreatsDto(CDET.id, THREAT.description) " +
            "from Case CDET " +
            "inner join CDET.framingMeeting FM " +
            "left join FM.selectedThreatsRel SELTHREATS " +
            "left join SELTHREATS.framingThreat THREAT " +
            "where CDET.id in (:casesIds) ")
    List<ExcelThreatsDto> getThreatsSup(@Param("casesIds") List<Long> lstCasesIds);


    @Query("select new com.umeca.model.entities.supervisor.ExcelSafetyFactorsDto(CDET.id, SAFTFACT.description) " +
            "from Case CDET " +
            "inner join CDET.framingMeeting FM " +
            "left join FM.selectedSafetyFactorsRel SELSAFTFACT " +
            "left join SELSAFTFACT.framingSafetyFactor SAFTFACT " +
            "where CDET.id in (:casesIds) ")
    List<ExcelSafetyFactorsDto> getSafetyFactorsSup(@Param("casesIds") List<Long> lstCasesIds);
    ...
    @Query("select new com.umeca.model.entities.supervisor.ExcelReferenceDto(CDET.id,FMREF.age, FMREF.isAccompaniment, FMREFRELS.name ) " +
            ...
            "left join FM.references FMREF " +
            "left join FMREF.relationship FMREFRELS " +
            "where CDET.id in (:casesIds) and FMREF.personType = com.umeca.model.entities.supervisor.FramingMeetingConstants.PERSON_TYPE_WITNESS ")
    List<ExcelReferenceDto> getWitnessSup(@Param("casesIds") List<Long> lstCasesIds);
    ...
    @Query("select new com.umeca.model.entities.supervisor.ExcelReferenceDto(CDET.id,FMREF.age, FMREF.isAccompaniment, FMREFRELS.name ) " +
            ...
            "left join FM.references FMREF " +
            "left join FMREF.relationship FMREFRELS " +
            "where CDET.id in (:casesIds) and FMREF.personType = com.umeca.model.entities.supervisor.FramingMeetingConstants.PERSON_TYPE_VICTIM ")
    List<ExcelReferenceDto> getVictimsSup(@Param("casesIds") List<Long> lstCasesIds);



    @Query("select new com.umeca.model.entities.supervisor.ExcelChannelingDto(CDET.id, CHTYPE.name,CH.isVolunteer, CH.name, CHINSTYPE.name) " +
            "from Channeling CH " +
            "inner join CH.caseDetention CDET " +
            "inner join CH.channelingType CHTYPE " +
            "inner join CH.institutionType CHINSTYPE " +
            "where CDET.id in (:casesIds) ")
    List<ExcelChannelingDto> getChannellingInfoSup(@Param("casesIds") List<Long> lstCasesIds);
    ...
    @Query("select new com.umeca.model.entities.supervisorManager.ExcelCaseInfoHearingFormatDto(" +
            ...
            "left join CDET.district DIST " +
            "left join HF.hearingImputed HFIMP " +
            "left join HFIMP.address ADDR " +
            "left join ADDR.location LOC " +
            "left join LOC.municipality MUNI " +
            "left join MUNI.state STATE " +
    ...
    @Query("select new com.umeca.model.entities.supervisor.ExcelCrimeDto(" +
            "CDET.id, " +
            "CRIME.name" +
            ") " +
            "from Case CDET " +
            "inner join CDET.hearingFormats HF " +
             "left join HF.crimeList HFCRIME " +
             "left join HFCRIME.crime CRIME " +
            "where CDET.id in (:casesIds)")
    List<ExcelCrimeDto> getCrimesHearingFormatInfo(@Param("casesIds") List<Long> lstCasesIds);



    @Query("select new com.umeca.model.entities.supervisor.ExcelArrangementDto(CDET.id, ARRAN.description) " +
            "from Case CDET " +
            "inner join CDET.hearingFormats HF " +
            "left join HF.assignedArrangements ASSIGNEDARR " +
            "left join ASSIGNEDARR.arrangement ARRAN " +
            "where CDET.id in (:casesIds)")
    List<ExcelArrangementDto> getArrangementsHearinFormat(@Param("casesIds") List<Long> lstCasesIds);

    @Query("select new com.umeca.model.entities.supervisor.ExcelContactsDto(CDET.id, CONTACT.liveWith) " +
            "from Case CDET " +
            "inner join CDET.hearingFormats HF " +
