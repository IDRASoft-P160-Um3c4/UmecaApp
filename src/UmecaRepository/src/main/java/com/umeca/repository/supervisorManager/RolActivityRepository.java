...
public interface RolActivityRepository extends JpaRepository<RolActivity, Long>{
    ...
    @Query("SELECT new com.umeca.model.dto.supervisorManager.RolActivityResponse(ra.id, ra.end, ra.start, ra.evaluator.id, ra.status, ra.place, ra.name) " +
            ...
            "WHERE ra.status <>:status AND " +
            "(ra.searchStart =:yearmonthStart OR ra.searchEnd =:yearmonthStart OR ra.searchStart =:yearmonthEnd OR ra.searchEnd =:yearmonthEnd)")
    List<RolActivityResponse> getAllActivitiesEvaluator(@Param("status")String status, @Param("yearmonthStart")int yearmonthStart, @Param("yearmonthEnd")int yearmonthEnd);
    ...
    @Query("SELECT DISTINCT new com.umeca.model.shared.SelectList(su.id, su.username, su.fullname) " +
            ...
            "WHERE ra.status <>:status AND " +
            "(ra.searchStart =:yearmonthStart OR ra.searchEnd =:yearmonthStart OR ra.searchStart =:yearmonthEnd OR ra.searchEnd =:yearmonthEnd)")
    List<SelectList> getAllValidEvaluators(@Param("status")String status, @Param("yearmonthStart")int yearmonthStart, @Param("yearmonthEnd")int yearmonthEnd);
}
